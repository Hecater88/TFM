...
public class Puzzle6 : MonoBehaviour {
		...
		void Update(){
			if (Input.GetKey(KeyCode.DownArrow)) {
				DataManager.DManager.SetCondition ("PuzzleMarmol", true);
				SceneController.SC.LoadSceneWithoutFade("MarmolComics");
			}
		...
		IEnumerator TransitionToNextQuestion(){
			...
			if (questionNumber < questions.Length) {
				...
				yield return new WaitForSeconds (timeBetweenQuestions);
				SceneController.SC.LoadSceneWithoutFade("Puzzle6");
	...
	public void ExitGame(){
		...
		SceneController.SC.FadeAndLoadScene ("MarmolComics");
	}
